...
# ============================================================
# 정산 매크로 v92.9 - 에러 해결 및 실장님 픽 디자인 통합본
# ============================================================
...
def copy_box(text, color_type="blue", box_id="default"):
    ...
    """
    components.html(html_code, height=box_height, key=f"comp_{box_id}")

st.set_page_config(page_title="정산 매크로 v92.9", layout="centered")
...
if st.session_state.page == 'settle':
    ...
    if bal_in > 0 and amount > 0:
        ...
        copy_box(b_msg, "green", box_id="balance_box")
    ...
    if amount > 0:
        copy_box(f"드래곤 테더정산 마크업 {m_fee}% {selected_m} / {fmt(amount)} / {fmt(markup)}", "yellow", box_id="markup_box")

    st.divider()
    st.markdown('<div class="label-header" style="color:#2ecc71;">06. TOP-UP 요청</div>', unsafe_allow_html=True)
    cl, cr = st.columns(2) 
    with cl: tm_rate = extract_int(st.text_input("탑업 빗썸 시세", key="tm_v"))
    with cr: t_usdt = extract_int(st.text_input("탑업 USDT 수량", key="tu_v"))
