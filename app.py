...
# [정산 매크로 v76 - 사이드바 직관 버튼 메뉴 시스템 Sync]
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("v76 Enterprise Sync")
...
if st.session_state.page == 'settle':
    ...
    current_rate = s_val if s_val > 0 else math.ceil(b_val * 1.045)
    st.code(f"1usdt = {current_rate:,} krw", language="text")
...
# ---------------------------------------------------------
    ...
    st.info("지갑 주소나 요율이 변경되면 여기서 수정하세요.")
