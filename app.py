...
# [정산 매크로 v76 - Noah 커스텀: 폰트 최적화 및 복사 버전]
...
if st.session_state.page == 'settle':
    ...
    c1, c2 = st.columns(2)
    with c1:
        b_val = extract_int(st.text_input("빗썸 시세", value="0", key="bithumb"))
    with c2:
        s_val = extract_int(st.text_input("수동 환율", value="0", key="manual"))
    ...
    for name, info in list(db.items()):
        with st.expander(f"🏢 {name} 수정/삭제"):
            ...
            if st.button(f"{name} 저장", key=f"s_{name}"):
                db[name] = {"wallet": u_wallet, "fee": u_fee}; save_data(db)
