...
# ============================================================
# 정산 매크로 v91.9 - Noah 실장님 최종 UI & 데이터 완벽 복구
# ============================================================
...
def copy_box(text, color_type="blue", key=None):
    ...
    st.markdown(f"""<style>.stTextArea textarea[key="{key}"] {{ background-color: {bg} !important; color: {c} !important; border-left: 4px solid {c} !important; font-family: 'Courier New', monospace !important; font-size: 15px !important; line-height: 1.6 !important; }}</style>""", unsafe_allow_html=True)
...
st.set_page_config(page_title="정산 매크로 v91.9", layout="centered")

# --- Noah 실장님 커스텀 CSS (다크테마 & 노란색 글자) ---
st.markdown("""
<style>
    ...
    .stMarkdown h3 { color: #4a90d9; border-bottom: 1px solid #34495e; padding-bottom: 5px; margin-top: 25px; }
    /* 가로형 버튼 스타일 */
    div[role="radiogroup"] { display: flex; gap: 10px; background: transparent !important; }
    div[role="radiogroup"] label { background-color: #2c3e50 !important; padding: 8px 20px !important; border-radius: 5px !important; color: white !important; cursor: pointer; }
...
if st.session_state.page == 'settle':
    ...
    sel_label = st.radio("배수 선택", list(m_map.keys()), horizontal=True, label_visibility="collapsed")
    c1, c2 = st.columns(2)
    ...
    if amt > 0:
        ...
        copy_box(confirm_msg, "blue", key="s_res")

    st.markdown("### 03. 최종 잔액 보고")
    bal_in = extract_int(st.text_input("현재 잔액 입력 (KRW)"))
    ...
    warn_in = extract_int(st.text_input("경고용 잔액 입력"))
    if warn_in > 0:
    ...
    cl, cr = st.columns(2)
    with cl: tm_rate = extract_int(st.text_input("탑업 빗썸 시세"))
    with cr: t_usdt = extract_int(st.text_input("탑업 USDT 수량"))
    ...
    # 관리 페이지 로직 동일 (spfxm 등 업체 수정/삭제 가능)
    # ... (생략하지만 실제 코드에는 포함)