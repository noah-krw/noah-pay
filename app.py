...
<style>
    ...
    .section-header .num { font-family: 'Space Mono', monospace; font-size: 0.75em; font-weight: 700; color: var(--hdr-color, #4a90d9); letter-spacing: 0.1em; opacity: 0.7; white-space: nowrap; }
    .section-header .title { font-family: 'Noto Sans KR', sans-serif; font-size: 0.92em; font-weight: 700; color: #d0dff0; letter-spacing: 0.08em; text-transform: uppercase; }
    .section-header .line { flex: 1; height: 1px; background: linear-gradient(90deg, rgba(var(--hdr-rgb, 74,144,217),0.3) 0%, transparent 100%); }
    div[data-baseweb="input"] { background-color: #0b1525 !important; border-radius: 7px !important; border: 1px solid rgba(74,144,217,0.25) !important; transition: border-color 0.2s ease !important; }
    div[data-baseweb="input"]:focus-within { border-color: rgba(74,144,217,0.7) !important; box-shadow: 0 0 0 2px rgba(74,144,217,0.1) !important; }
    input { color: #dce8f5 !important; font-weight: 400 !important; font-family: 'Space Mono', monospace !important; font-size: 1.1em !important; background: transparent !important; caret-color: #4a90d9 !important; }
    label[data-testid="stWidgetLabel"] > div > p { color: #6a8aaa !important; font-size: 0.82em !important; font-family: 'Space Mono', monospace !important; letter-spacing: 0.06em !important; text-transform: uppercase !important; }
    [data-testid="stRadio"] label { font-family: 'Space Mono', monospace !important; font-size: 0.88em !important; color: #7aa8cc !important; }
    div[data-baseweb="select"] > div { background-color: #0b1525 !important; border: 1px solid rgba(74,144,217,0.25) !important; border-radius: 7px !important; color: #a8c7e8 !important; font-family: 'Space Mono', monospace !important; }
    .rate-text { font-family: 'Space Mono', monospace; color: #5dade2; font-size: 0.95em; margin: 8px 0 6px; padding: 8px 14px; background: rgba(93,173,226,0.07); border-radius: 6px; letter-spacing: 0.04em; }
    hr { border-color: rgba(74,144,217,0.1) !important; margin: 24px 0 !important; }
    div[data-testid="stSidebar"] div:has(> button[key="reset_inputs"]) button { background: linear-gradient(135deg, #0d2a1a, #0a1f2e) !important; border-color: rgba(46,204,113,0.4) !important; color: #2ecc71 !important; }
    div[data-testid="stSidebar"] div:has(> button[key="reset_inputs"]) button:hover { background: #2ecc71 !important; color: #000 !important; }
...
if st.session_state.page == 'settle':
    ...
    html = (
        ...
        "background:#2ecc71;box-shadow:0 0 7px #2ecc71;animation:blink 1.5s infinite;'></span>"
        ...
        "<div style='font-family:Space Mono,monospace;font-size:1.8em;font-weight:700;color:#ffffff;letter-spacing:0.03em;'>" + bithumb_str + "</div>"
        "<div style='font-family:Space Mono,monospace;font-size:0.68em;color:#5dade2;letter-spacing:0.1em;'>" + fetched_time + "</div>"
        "</div>"
...
# ══════════════════════════════════════════════════════════
    ...
    topup_html = (
        ...
        "<div style='display:flex;align-items:center;gap:8px;'>"
        "<span style='display:inline-block;width:7px;height:7px;border-radius:50%;"
        "background:#2ecc71;box-shadow:0 0 7px #2ecc71;animation:blink2 1.5s infinite;'></span>"
        "<span style='font-family:Space Mono,monospace;font-size:0.68em;color:#5dade2;letter-spacing:0.1em;'>BITHUMB &nbsp; USDT / KRW</span>"
        "</div>"
        "<div style='display:flex;flex-direction:column;align-items:center;gap:3px;'>"
        "<div style='font-family:Space Mono,monospace;font-size:1.8em;font-weight:700;color:#ffffff;letter-spacing:0.03em;'>" + bithumb_str + "</div>"
    ...
    if t_rate > 0:
        st.markdown(
            f"<div style='font-family:Space Mono,monospace;color:#5dade2;"
            f"font-size:0.95em;letter-spacing:0.04em;margin-bottom:12px;"
            f"padding:8px 14px;background:rgba(93,173,226,0.07);border-radius:6px;'>"
    ...
    if tu_amt > 0 and t_rate > 0:
        ...
        t_msg = (f"mid : {selected_m}\n"
                 f"top-up usdt : {fmt(int(tu_amt))} usdt\n"
...
# ══════════════════════════════════════════════════════════
    ...
    <div style="margin-top:8px;margin-bottom:14px;display:flex;align-items:center;gap:12px;
        ...
        border-left:3px solid #4a90d9;overflow:hidden;position:relative;">
        <div style="position:absolute;top:0;left:0;right:0;bottom:0;
            background:linear-gradient(90deg,rgba(74,144,217,0.07) 0%,transparent 100%);pointer-events:none;"></div>
        <span style="font-family:'Space Mono',monospace;font-size:0.75em;font-weight:700;color:#4a90d9;letter-spacing:0.1em;opacity:0.7;">MY</span>
        <span style="font-family:'Noto Sans KR',sans-serif;font-size:0.92em;font-weight:700;color:#d0dff0;letter-spacing:0.08em;text-transform:uppercase;">내 지갑 주소</span>
    ...
    <div style="margin-top:32px;display:flex;align-items:center;gap:12px;
        ...
        <div style="position:absolute;top:0;left:0;right:0;bottom:0;
            background:linear-gradient(90deg,rgba(168,85,247,0.07) 0%,transparent 100%);pointer-events:none;"></div>
        <span style="font-family:'Space Mono',monospace;font-size:0.75em;font-weight:700;color:#a855f7;letter-spacing:0.1em;opacity:0.7;">NEW</span>
        <span style="font-family:'Noto Sans KR',sans-serif;font-size:0.92em;font-weight:700;color:#d0dff0;letter-spacing:0.08em;text-transform:uppercase;">업체 추가</span>
    ...
    <div style="margin-top:32px;margin-bottom:14px;display:flex;align-items:center;gap:12px;
        ...
        <div style="position:absolute;top:0;left:0;right:0;bottom:0;
            background:linear-gradient(90deg,rgba(46,204,113,0.07) 0%,transparent 100%);pointer-events:none;"></div>
        <span style="font-family:'Space Mono',monospace;font-size:0.75em;font-weight:700;color:#2ecc71;letter-spacing:0.1em;opacity:0.7;">LIST</span>
        <span style="font-family:'Noto Sans KR',sans-serif;font-size:0.92em;font-weight:700;color:#d0dff0;letter-spacing:0.08em;text-transform:uppercase;">등록 업체 관리</span>
